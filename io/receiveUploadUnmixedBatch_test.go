...
func TestReceivePostNewBatch_Errors(t *testing.T) {
	...
	r, err := round.New(grp, instance.GetStorage(), roundID, []phase.Phase{precompReveal, realDecrypt}, responseMap, topology, topology.GetNodeAtIndex(0), batchSize, instance.GetRngStreamGen(), nil, "0.0.0.0", nil, nil, nil, nil)
	if err != nil {
...
func TestReceivePostNewBatch(t *testing.T) {
	...
	r, err := round.New(grp, instance.GetStorage(), roundID, []phase.Phase{realDecrypt}, responseMap, topology, topology.GetNodeAtIndex(0), batchSize, instance.GetRngStreamGen(), nil, "0.0.0.0", nil, nil, nil, nil)
	if err != nil {
		t.Errorf("Failed to create new round: %+v", err)
	}
	instance.GetRoundManager().AddRound(r)
