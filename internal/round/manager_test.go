...
func TestManager(t *testing.T) {
	roundID := id.Round(58)
	round, err := New(grp, roundID, nil, nil, connect.NewCircuit([]*id.ID{{}}), &id.ID{}, 1, fastRNG.NewStreamGenerator(10000, uint(runtime.NumCPU()),
...
func TestManager_GetPhase(t *testing.T) {
	...
	round, err = New(grp, roundID, phases, nil, connect.NewCircuit([]*id.ID{{}}), &id.ID{}, 1, fastRNG.NewStreamGenerator(10000, uint(runtime.NumCPU()),
		csprng.NewSystemRNG), nil, "0.0.0.0", nil, nil, nil, nil)
	if err != nil {
		t.Errorf("Failed to create new round: %+v", err)
	}
