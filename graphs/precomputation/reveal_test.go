...
//		}
//
//		if !reflect.DeepEqual(rs.CypherMsg.Get(b).Bytes(), expected[2]) {
...
//		}
//
//		if !reflect.DeepEqual(rs.CypherAD.Get(b).Bytes(), expected[3]) {
...
func TestRevealStream_Input_OutOfGroup(t *testing.T) {
	...
	if err == nil {
		t.Errorf("DecryptStream.Input() did nto return an error when out of group")
	}
}
