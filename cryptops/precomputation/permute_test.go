...
func TestPermute(t *testing.T) {
	...
	grp := cyclic.NewGroup(large.NewInt(107), large.NewInt(27), large.NewInt(23))
	...
	inMessages = append(inMessages, &PrecomputationSlot{
		Slot:                         uint64(0),
	...
	for i := uint64(0); i < batchSize; i++ {
		...
		if act.MessageCypher.Cmp(expected[i].MessageCypher) != 0 {
		...
		if act.AssociatedDataCypher.Cmp(expected[i].AssociatedDataCypher) != 0 {
		...
		if act.MessagePrecomputation.Cmp(expected[i].MessagePrecomputation) != 0 {
			t.Errorf("Test of Precomputation Permute's cryptop failed Message"+
				"CypherText Test on index: %v\n\tExpected: %#v\n\tActual:   %#v", i,
				expected[i].MessagePrecomputation.Text(10),
				act.MessagePrecomputation.Text(10))
		}
		if act.AssociatedDataPrecomputation.Cmp(expected[i].AssociatedDataPrecomputation) != 0 {
			t.Errorf("Test of Precomputation Permute's cryptop failed Recipient"+
				"CypherText Test on index: %v\n\tExpected: %#v\n\tActual:   %#v", i,
